...
final class DataGovernanceServiceTest extends TestCase
{
    ...
    public function testAccountExportIsPersistedAndQueuedAtomically(): void
    {
        ...
        $store->expects(self::once())
            ...
            ->willReturnCallback(
                function () use ($transactions): void {
    ...
    public function testAccountErasureIsBlockedWhileTheUserOwnsAHome(): void
    {
        ...
        $store->method('ownedHomeIds')
            ->willReturn([self::HOME_ID]);
        $this->expectException(Problem::class);
        $this->service($store)
            ->requestAccountErasure($this->identity());
    }
    ...
    public function testDelegatedPermissionDoesNotBypassHomeErasureOwnershipSafeguard(): void
    {
        ...
        $homeStore->expects(self::once())
            ->method('permissionDecision')
            ->with(
                self::HOME_ID,
                HomeAuthorization::MANAGER,
                HomePermission::DATA_ERASURE,
            )
            ->willReturn(
                true,
            );
    ...
    public function testCrossUserCannotConsumeAnAccountExport(): void
    {
        ...
        $store->expects(self::never())
            ->method('consumeDownload');
        $this->expectException(Problem::class);
        $this->downloadService($store)
            ->download($this->identity(), self::REQUEST_ID, 'secret');
    }
    ...
    {
        $store = $this->createStub(DataGovernanceStore::class);
        $store->method('nextQueuedRequest')
            ->willReturn(null);
    ...
    public function testProcessorRecordsABoundedFailureTransition(): void
    {
        ...
        $exports->method('generate')
            ->willThrowException(
                new \RuntimeException('fixture export failed'),
            );
        $processor = new DataGovernanceProcessor(
            $store,
            new HomeFixedClock(
