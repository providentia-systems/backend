...
final class AiSettingsPrivacyTest extends TestCase
{
    ...
    public function testSettingsDistinguishDirectTransitFromExplicitEncryptedStorage(): void
    {
        $settings = $this->aiService()
            ->settings($this->identity(), self::HOME_ID);
    ...
    public function testDirectExtractionStoresMetadataButNeverCreatesAPrivateMediaObject(): void
    {
        ...
        $store->method('targetExists')
            ->with(self::HOME_ID, 'receipt', self::RECEIPT_ID)
            ->willReturn(
                true,
            );
        $store->method('settings')
            ->with(self::HOME_ID)
            ->willReturn(
                [
                'mode' => 'server_proxy',
        ...
        $provider = new class implements AiProvider
        {
            ...
            public function extract(ExtractionRequest $request): ExtractionOutcome
            {
                return new ExtractionOutcome(
                    AiSettingsPrivacyTest::emptyDocument(),
                    [
                        'inputTokens' => 1,
                        'outputTokens' => 1,
                        'totalTokens' => 2,
    ...
    public function testHttpBoundaryAcceptsImageAndTwoRepeatedImagesArrayParts(): void
    {
        ...
        $store = $this->createMock(AiStore::class);
        $store->method('targetExists')
            ->with(self::HOME_ID, 'receipt', null)
            ->willReturn(
                true,
            );
        $store->method('settings')
        ...
        $ids = $this->createStub(UuidGenerator::class);
        $ids->method('generate')
            ->willReturn('01912345-6789-7abc-ddef-0123456789ab');
        $schema = new ExtractionSchema();
        $service = new AiService(
            $store,
            $maturity,
            ...
            $this->privateMedia(
                $maturity,
                $this->createStub(MediaStorage::class),
            ),
            $this->authorization(),
            $ids,
            $this->clock(),
        ...
        $request = fn(): ServerRequest => new ServerRequest(
            ...
            [],
            new Uri(
                'https://app.example.test/api/v1/homes/' . self::HOME_ID . '/ai/extractions',
            ),
            'POST',
            'php://memory',
        )->withAttribute(
        ...
        foreach (
            ...
            try {
                ...
                self::fail(
                    'An extraction multipart request outside the contract was accepted.',
                );
    ...
    public function testHttpBoundaryErasesReadBytesAndClosesEveryStreamWhenLaterValidationFails(): void
    {
        ...
        try {
            ...
            self::fail(
                'An undersized additional observation was accepted.',
            );
        } catch (Problem $problem) {
    ...
    public function testHttpBoundaryErasesAllBuffersAndClosesStreamsWhenTheProviderFails(): void
    {
        ...
        try {
            ...
            self::fail(
                'A failed provider request was reported as successful.',
            );
        } catch (Problem $problem) {
    ...
    public function testProfileCredentialRevocationIsPrivacySafeAndDoesNotConsultPolicy(): void
    {
        ...
        $ids = $this->createStub(UuidGenerator::class);
        $ids->method('generate')
            ->willReturn('01912345-6789-7abc-cdef-0123456789ab');
        $profile = $this->aiService($maturity, $ids)
            ->revokeProviderProfileCredential(
                $this->identity(),
                self::HOME_ID,
                self::PROFILE_ID,
                3,
    ...
    public function testStoredExtractionRejectsTruthyStringsAsTransmissionConsent(): void
    {
        $request = new ServerRequest(
            ...
            [],
            new Uri(
                'https://app.example.test/api/v1/homes/' . self::HOME_ID . '/ai/extractions/from-media',
            ),
            'POST',
            'php://memory',
        )->withAttribute(
        ...
        try {
            ...
            self::fail(
                'A truthy JSON string was accepted as transmission consent.',
            );
        } catch (Problem $problem) {
    ...
    public function testAlreadyClearProfileCredentialRevocationIsAnIdempotentCurrentRevisionRead(): void
    {
        ...
        $maturity->expects(self::never())
            ->method('revokeProviderProfileCredential');
        $result = $this->aiService($maturity)
            ->revokeProviderProfileCredential(
                $this->identity(),
                self::HOME_ID,
                self::PROFILE_ID,
    ...
    public function testProfileCredentialRevocationConcealsAWrongHomeProfile(): void
    {
        ...
        try {
            ...
            self::fail(
                'A provider profile from another home was exposed.',
            );
        } catch (Problem $problem) {
    ...
    public function testPrivateProfileCreationBindsOwnerScopedAssociatedDataAndHidesOwnerIds(): void
    {
        ...
        $maturity->method('providerProfile')
            ->willReturn(null);
        $saved = null;
        $maturity->method('saveProviderProfile')
            ->willReturnCallback(
        ...
        $cipher->method('available')
            ->willReturn(true);
        $cipher->expects(self::once())
            ->method('encrypt')
            ->with(
                'compatible-secret-0001',
                'providentia-ai-profile:v2:' . self::HOME_ID . ':' . self::USER_ID . ':'
                . self::PROFILE_ID,
            )
            ->willReturn(
                [
        ...
        $result = $this->profileService($maturity, $cipher, HomeAuthorization::MANAGER)
            ->putProviderProfile(
                $this->identity(),
                self::HOME_ID,
                self::PROFILE_ID,
    ...
    public function testHomeSharedProfileWritesAreADeliberateHomeOwnerChoice(): void
    {
        ...
        $maturity->method('providerProfile')
            ->willReturn(null);
        $maturity->expects(self::never())
            ->method('saveProviderProfile');
        try {
            $this->profileService($maturity, null, HomeAuthorization::MANAGER)
                ->putProviderProfile(
                    $this->identity(),
                    self::HOME_ID,
                    self::PROFILE_ID,
            ...
            self::fail(
                'A non-owner created a home-shared provider profile.',
            );
        } catch (Problem $problem) {
            self::assertSame(403, $problem->status);
        }
        $owning = $this->createStub(AiMaturityStore::class);
        $owning->method('providerProfile')
            ->willReturn(null);
        $saved = null;
        $owning->method('saveProviderProfile')
            ->willReturnCallback(
                static function (array $profile) use (&$saved): bool {
                    $saved = $profile;
                    return true;
        ...
        $result = $this->profileService($owning, null, HomeAuthorization::OWNER)
            ->putProviderProfile(
                $this->identity(),
                self::HOME_ID,
                self::PROFILE_ID,
    ...
    public function testAnotherMembersPrivateProfileIsUnaddressableByEveryProfileWrite(): void
    {
        ...
        foreach ($writes as $write) {
            try {
                $write();
                self::fail(
                    'Another member\'s private provider profile was addressable.',
                );
            } catch (Problem $problem) {
                self::assertSame(404, $problem->status);
            }
    ...
    public function testHomeSharedProfileDeletionRequiresTheOwnerWhilePrivateOnesStayManageable(): void
    {
        ...
        try {
            ...
            self::fail(
                'A non-owner deleted a home-shared provider profile.',
            );
        } catch (Problem $problem) {
            self::assertSame(403, $problem->status);
        }
        ...
        $owned->method('providerProfile')
            ->willReturn($own);
        $owned->method('orchestrationPolicy')
            ->willReturn(null);
        $owned->expects(self::once())
            ->method('revokeProviderProfile')
            ->with(
                self::HOME_ID,
                self::PROFILE_ID,
        ...
        $this->profileService($owned, null, HomeAuthorization::MANAGER)
            ->removeProviderProfile(
                $this->identity(),
                self::HOME_ID,
                self::PROFILE_ID,
    ...
    public function testOwnerScopeConversionCannotCarryTheOldCredentialCiphertext(): void
    {
        ...
        $maturity->expects(self::never())
            ->method('saveProviderProfile');
        try {
            $this->profileService($maturity, null, HomeAuthorization::OWNER)
                ->putProviderProfile(
                    $this->identity(),
                    self::HOME_ID,
                    self::PROFILE_ID,
            ...
            self::fail(
                'An owner-scope change silently reused a differently bound ciphertext.',
            );
        } catch (Problem $problem) {
            self::assertSame(422, $problem->status);
    ...
    public function testProfileEndpointWritePolicyMatrix(): void
    {
        $rejected = [
            ...
            [
                ...
                'openai-compatible',
                'compatible-secret-0001',
                'http://vision.example.test/v1/chat/completions',
            ],
            ...
            [
                ...
                'openai-compatible',
                'compatible-secret-0001',
                'https://user:pw@vision.example.test/v1',
            ],
            [
                true,
                'openai-compatible',
                'compatible-secret-0001',
                self::COMPATIBLE_ENDPOINT . '?redirect=x',
            ],
            [
                true,
                'openai-compatible',
                'compatible-secret-0001',
                self::COMPATIBLE_ENDPOINT . '#fragment',
            ],
            ...
            [
                ...
                'ollama',
                null,
                'http://ollama.lan:11434/api/chat',
            ],
        ];
        foreach ($rejected as [$lanPolicy, $provider, $credential, $endpoint]) {
            ...
            try {
                ...
                self::fail(
                    'An endpoint outside the write policy was accepted: ' . $endpoint,
                );
            } catch (Problem $problem) {
        ...
        $accepted = [
            ...
            [
                ...
                'ollama',
                null,
                'https://ollama.example.test/api/chat',
            ],
            [
                true,
                'ollama',
    ...
    public function testScansPreferTheRequestingPersonsPrivateProfileOverTheHomeSharedOne(): void
    {
        ...
        $attempts = [];
        $maturity->method('appendExtractionAttempt')
            ->willReturnCallback(
                static function (
                    string $extractionId,
                    int $position,
        ...
        $ids = $this->createStub(UuidGenerator::class);
        $ids->method('generate')
            ->willReturn('01912345-6789-7abc-edef-0123456789ab');
        $schema = new ExtractionSchema();
        $service = new AiService(
            $store,
            $maturity,
    ...
    public function testProfilePutHttpBoundaryForwardsOwnerScopeAndEndpoint(): void
    {
        ...
        $request = new ServerRequest(
            ...
            [],
            new Uri(
                'https://app.example.test/api/v1/homes/' . self::HOME_ID . '/ai/profiles/'
                    . self::PROFILE_ID,
            ),
            'PUT',
            'php://memory',
    ...
    private function syntheticProvider(
        string $id,
    ...
    private function directExtractionService(
        ...
        $ids = $this->createStub(UuidGenerator::class);
        $ids->method('generate')
            ->willReturn('01912345-6789-7abc-fdef-0123456789ab');
        $schema = new ExtractionSchema();
        return new AiService(
            $store,
            $maturity,
