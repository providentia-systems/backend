...
final class PurchasingHandlerTest extends TestCase
{
    public function testOrdinaryHttpUnresolvedDecisionUsesExpectedRevision(): void
    {
        ...
        $purchases->expects(self::once())
            ...
            ->willReturn(['id' => $receiptId, 'status' => 'committed', 'revision' => 5]);
        $purchases->expects(self::once())
            ->method('receiptLine')
            ->with($homeId, $receiptId, $lineId)
            ->willReturn([
                'id' => $lineId,
        ...
        $service = new PurchasingService(
            ...
            new HomeFixedClock(new DateTimeImmutable('2026-08-10T12:00:00+00:00')),
            new RecordingTransactionManager(),
        );
        $request = (new ServerRequest(
            [],
            [],
            new Uri('https://app.example.test/api/v1/homes/' . $homeId . '/receipts/' . $receiptId),
            'POST',
            'php://memory',
        ))
            ->withAttribute('homeId', $homeId)
            ->withAttribute('receiptId', $receiptId)
            ->withAttribute('lineId', $lineId)
            ->withAttribute(
                BearerAuthenticationMiddleware::ATTRIBUTE,
                new AuthenticatedIdentity($userId, 'session', 'device', $homeId, []),
            )
            ->withParsedBody(['expectedRevision' => 2]);
    ...
    public function testOrdinaryHttpUnresolvedDecisionRejectsSchemaInvalidBodiesBeforeMutation(): void
    {
        ...
        foreach (
            ...
            try {
                ...
                self::fail('A schema-invalid unresolved decision body was accepted.');
