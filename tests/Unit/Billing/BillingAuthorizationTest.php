...
final class BillingAuthorizationTest extends TestCase
{
    ...
    public function testHomeBillingPermissionDefaultsAreLeastPrivilege(string $role, string $permission): void
    {
        $homes = $this->createStub(HomeStore::class);
        $homes->method('membership')->willReturn(['status' => 'active', 'role' => $role]);
        $authorization = new BillingAuthorization(new HomeAuthorization($homes));
        ...
        try {
            ...
            self::fail('A least-privilege billing permission was bypassed.');
    ...
    public function testPersistedHomePolicyCanGrantBillingReadToAMember(): void
    {
        ...
        $homes->method('membership')->willReturn(['status' => 'active', 'role' => 'member']);
        $homes->expects(self::once())->method('permissionDecision')->with(
            'home-1',
            'member',
    ...
    {
        $homes = $this->createStub(HomeStore::class);
        $homes->method('membership')->willReturn(['status' => 'active', 'role' => 'manager']);
        $homes->method('permissionDecision')->willReturn(false);
        $authorization = new BillingAuthorization(new HomeAuthorization($homes));
    ...
    {
        return new AuthenticatedIdentity('user-1', 'session-1', 'device-1', null, []);
    }
}
