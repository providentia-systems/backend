...
final class CatalogContributionImageServiceTest extends TestCase
{
    ...
    public function testUploadStoresOnlySanitizedEncryptedMediaAndReturnsPrivateSourceBinding(): void
    {
        ...
        $images->expects(self::once())
            ->method('saveQuarantineImage')
            ->with(
                self::CONTRIBUTION_ID,
        ...
        $audit->expects(self::once())
            ...
            ->with(
                ...
                self::HOME_ID,
                self::callback(
                    static fn(string $details): bool => str_contains(
                        $details,
        ...
        $submission = $this->service($contributions, $images, audit: $audit)
            ->upload(
                $this->homeIdentity(),
                self::HOME_ID,
                self::CONTRIBUTION_ID,
    ...
    public function testExactUploadReplayDoesNotDuplicateQuarantineOrAudit(): void
    {
        ...
        $contributions->method('consent')
            ->willReturn($this->consent());
        $contributions->method('createContribution')
            ->willReturn(
                [
                'outcome' => 'replayed',
                'record' => $this->record(),
        ...
        $images = $this->createMock(CatalogContributionImageStore::class);
        $images->expects(self::never())
            ->method('saveQuarantineImage');
        $images->method('quarantineImage')
            ->willReturn(
                ['assetDigest' => hash('sha256', self::SANITIZED)],
            );
        $audit = $this->createMock(CatalogAuditRecorder::class);
        $audit->expects(self::never())
            ->method('recordAudit');
        $submission = $this->service($contributions, $images, audit: $audit)
            ->upload(
                $this->homeIdentity(),
    ...
    public function testUploadFailsClosedForAStaleConsentBeforeMediaProcessing(): void
    {
        ...
        $sanitizer->expects(self::never())
            ->method('sanitize');
        $this->expectException(Problem::class);
        $this->expectExceptionMessage(
            'Product-image sharing is not currently enabled.',
        );
        $this->service(
            $contributions,
    ...
    public function testCrossHomeSourceIsPrivacySafeAndNeverCreatesAContribution(): void
    {
        ...
        $contributions->method('consent')
            ->willReturn($this->consent());
        $contributions->expects(self::never())
            ->method('createContribution');
        $sources = $this->createStub(CatalogContributionSourceReader::class);
        $sources->method('activeHomeProduct')
            ->willReturn(null);
        try {
            $this->service(
                $contributions,
    ...
    public function testMultipartHttpBoundaryRequiresExplicitConfirmationAndReturnsCreatedContribution(): void
    {
        ...
        $request = new ServerRequest(
            ...
            [],
            new Uri(
                'https://api.example.test/api/v1/homes/' . self::HOME_ID . '/catalog-contributions/images',
            ),
            'POST',
            'php://memory',
        )->withAttribute(
        ...
        foreach (
            ...
            $request->withParsedBody(
                array_replace($validBody, ['submissionConfirmed' => '1']),
            ),
            $request->withParsedBody(
                array_replace($validBody, ['unexpected' => 'field']),
            ),
            $request->withParsedBody(
                array_replace(
                    $validBody,
    ...
    public function testPublicDigestMustMatchTheExactLowercasePathValue(): void
    {
        ...
        try {
            $this->service(
                ...
                ->publicAsset(
                    strtoupper(hash('sha256', self::SANITIZED)),
                );
            self::fail(
                'An uppercase public-asset digest was normalized.',
            );
        } catch (Problem $problem) {
            self::assertSame(404, $problem->status);
        }
    ...
    public function testPublicationHandlerRejectsFieldsOutsideTheClosedContract(): void
    {
        ...
        $request = new ServerRequest(
            ...
            [],
            new Uri(
                'https://app.example.test/api/v1/catalog-contributions/' . self::CONTRIBUTION_ID
                    . '/image-publication',
            ),
            'PUT',
            'php://memory',
        ...
        try {
            ...
            self::fail(
                'An image-publication request outside the closed JSON contract was accepted.',
            );
        } catch (Problem $problem) {
            self::assertSame(422, $problem->status);
        }
    ...
    public function testDecryptionFailureReturnsAStableUnavailableProblem(): void
    {
        ...
        $cipher->method('decrypt')
            ->willThrowException(new \RuntimeException('secret detail'));
        try {
            $this->service(
                $this->createStub(CatalogContributionStore::class),
                $images,
    ...
    public function testBinaryPreviewBoundaryUsesNoStoreAndIntegrityHeaders(): void
    {
        ...
        $request = new ServerRequest(
            ...
            [],
            new Uri(
                'https://api.example.test/api/v1/catalog-contributions/' . self::CONTRIBUTION_ID
                    . '/image-preview?expectedRevision=1',
            ),
            'GET',
            'php://memory',
        )->withAttribute(
        ...
        self::assertSame(
            hash('sha256', self::SANITIZED),
            $response->getHeaderLine('X-Content-SHA256'),
        );
    ...
    public function testApprovedImagePublishesToAnExplicitProductAndReturnsDurableLink(): void
    {
        ...
        $images = $this->createMock(CatalogContributionImageStore::class);
        $images->method('imageForPublication')
            ->willReturn($source);
        $images->expects(self::once())
            ->method('savePublicAsset')
            ->willReturn(
                [
                'id' => self::ASSET_ID,
                'assetDigest' => hash('sha256', self::SANITIZED),
        ...
        $images->expects(self::once())
            ->method('linkPublication')
            ->willReturn(true);
        $images->expects(self::once())
            ->method('deleteQuarantineImage')
            ->with(
                self::CONTRIBUTION_ID,
            );
        $images->method('publication')
            ->willReturn($this->publication());
        $catalog = $this->createStub(CatalogStore::class);
        $catalog->method('product')
            ->willReturn(
                [
                'id' => self::PRODUCT_ID,
        ...
        $governanceStore->expects(self::once())
            ->method('putIcon')
            ->willReturn(
                ['id' => self::ICON_ID, 'revision' => 3],
            );
        $audit = $this->createMock(CatalogAuditRecorder::class);
        $audit->expects(self::once())
            ->method('recordAudit')
    ...
    private function service(
        ...
        if ($cipher === null) {
            ...
            $cipher->method('encrypt')
                ->willReturn(
                    new EncryptedCatalogImage('ciphertext', str_repeat('n', 24), 1),
                );
            $cipher->method('decrypt')
                ->willReturn(self::SANITIZED);
        }
        $catalog ??= $this->createStub(CatalogStore::class);
        ...
        $ids->method('generate')
            ->willReturn(self::ASSET_ID);
        $clock = new HomeFixedClock(
    ...
    private function operatorIdentity(string $role): AuthenticatedIdentity
    {
        return new AuthenticatedIdentity(
            ...
            null,
            [$role],
            \ProvidentiaTest\Support\AccessFixture::administratorPermissions([$role]),
        );
    }
}
