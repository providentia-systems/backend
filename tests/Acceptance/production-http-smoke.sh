...
request root / 404
request removed-login-page \
  /login-links/admin/01912345-6789-7abc-8def-0123456789ab 404
...
grep -Eq '"status"[[:space:]]*:[[:space:]]*404' "$evidence_directory/removed-login-page.body"
if grep -Eiq '<(!doctype|html|form|script)' "$evidence_directory/removed-login-page.body"; then
  printf 'The removed login route exposed browser markup.\n' >&2
  exit 1
fi
