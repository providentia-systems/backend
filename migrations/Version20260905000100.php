...
final class Version20260905000100 extends AbstractMigration
{
    ...
    {
        return ('Pre-release group authorization, OTP identities, profiles, geography and'
            . ' policy records');
    }
    ...
    public function up(Schema $schema): void
    {
        ...
        $emails->addForeignKeyConstraint(
            'users',
            ['user_id'],
            ['id'],
            ['onDelete' => 'CASCADE'],
        ...
        $administrators->addColumn(
            'reviewer_user_id',
            Types::STRING,
            ['length' => 36, 'notnull' => false],
        );
        $administrators->addColumn('created_at', Types::DATETIME_IMMUTABLE);
        $administrators->addForeignKeyConstraint(
            'users',
            ['user_id'],
            ['id'],
            ['onDelete' => 'CASCADE'],
        ...
        $acceptances->addForeignKeyConstraint(
            'users',
            ['user_id'],
            ['id'],
            ['onDelete' => 'CASCADE'],
    ...
    public function postUp(Schema $schema): void
    {
        ...
        $body = (string) file_get_contents(
            dirname(__DIR__) . '/resources/policies/default-privacy.txt',
        );
        $this->connection->insert(
            'privacy_policies',
            [
                'id' => $policyId,
                'country_code' => null,
                'title' => 'Providentia privacy notice',
                'body' => $body,
                'status' => 'published',
                'revision' => 1,
                'published_at' => $at,
        ...
        $countries = json_decode(
            (string) file_get_contents(
                dirname(__DIR__) . '/resources/reference/countries.json',
            ),
            true,
            32,
            JSON_THROW_ON_ERROR,
        ...
        foreach ($countries as $country) {
            $this->connection->insert(
                ...
                [
                    ...
                    'timezones_json' => json_encode($country['timezones'], JSON_THROW_ON_ERROR),
                    'source_version' => 'v3.2-export.7',
                    'active' => 1,
                ],
            );
            $this->connection->insert(
                ...
                [
                    ...
                    'default_timezone' => $country['timezones'][0] ?? 'UTC',
                    'policy_id' => $policyId,
                    'revision' => 1,
                    'updated_at' => $at,
                ],
        ...
        $stream = fopen(
            dirname(__DIR__) . '/resources/reference/states.csv',
            'rb',
        );
        if ($stream === false) {
    ...
    public function down(Schema $schema): void
    {
        ...
        foreach (
            ...
            $schema->getTable('user_profiles')
                ->dropColumn($column);
        }
        foreach (['description', 'latitude', 'longitude'] as $column) {
            $schema->getTable('homes')
                ->dropColumn($column);
        }
        foreach (
            [
