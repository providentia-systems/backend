    ...
    public function request(array $input, string $ip): array
    {
        ...
        if (
            ...
            throw new Problem(
                ...
                'A persistent installation UUID is required.',
            );
        }
        ...
        if (
            !in_array($transport, ['web', 'native'], true) || $device === ''
            || mb_strlen($device) > 120 || $platform === '' || strlen($platform) > 40
        ) {
            throw new Problem(
                422,
                'Invalid device',
                'Supply the device name, platform and session transport.',
            );
        }
