    ...
    public function issue(array $challenge): void
    {
        ...
        $this->connection->executeStatement(
            ('UPDATE email_code_challenges SET consumed_at = :now
             WHERE '
                . 'email = :email AND purpose = :purpose AND consumed_at IS NULL'),
            [
    ...
    public function consume(
        ...
        if ($this->connection->isTransactionActive()) {
            throw new \LogicException(
                'Code verification must precede the application transaction.',
            );
        }
        return $this->connection->transactional(
            function () use ($id, $codeHash, $bindingHash, $purpose, $now): ?array {
                $changed = $this->connection->executeStatement(
                    ('UPDATE email_code_challenges SET attempts = attempts + 1'
                        . '
                 WHERE id = :id AND purpose = :purpose AND binding_hash'
                        . ' = :binding
                   AND attempts < 5 AND consumed_at IS NULL '
                        . 'AND expires_at > :now'),
                    [
                        'id' => $id,
                        'purpose' => $purpose,
