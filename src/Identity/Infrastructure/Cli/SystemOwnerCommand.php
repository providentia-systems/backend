...
final class SystemOwnerCommand extends Command
{
    ...
    protected function execute(
        ...
        $existing = $this->connection->fetchOne(
            'SELECT email FROM system_owner_bootstrap WHERE singleton_id = 1',
        );
        if ($existing !== false) {
            $output->writeln(
                $existing === $email
                    ? 'This system owner is already configured.'
                    : ('<error>The system owner is already configured; it cannot be replaced by '
                    . 'bootstrap.</error>'),
            );
        ...
        $output->writeln(
            'System owner authorized. Sign in to Providentia Admin using the emailed code.',
        );
