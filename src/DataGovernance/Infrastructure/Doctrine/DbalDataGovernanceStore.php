...
{
    private const SELECT_REQUEST = ('SELECT id, request_kind AS requestKind, scope_type AS scopeType,'
        . '
                subject_user_id AS subjectUserId, home_id AS homeId,'
        . '
                requested_by_user_id AS requestedByUserId, status, '
        . 'revision,
                retained_data_disclosure_json AS '
        . 'retainedDataDisclosure,
                artifact_reference AS '
        . 'artifactReference,
                artifact_nonce AS artifactNonce, '
        ...
        . '
                completed_at AS completedAt, cancelled_at AS '
        . 'cancelledAt,
                created_at AS createdAt, updated_at AS '
        . 'updatedAt
    ...
    public function isSystemOwner(string $userId): bool
    {
        return (bool) $this->connection->fetchOne(
            'SELECT user_id FROM system_owner_bootstrap WHERE user_id = ?',
            [$userId],
        );
    }
    ...
    public function createRequest(
        ...
        try {
            ...
            throw new \DomainException(
                'An active request of this kind already exists for the scope.',
            );
        }
    }
    ...
    public function requestsForUser(
        ...
        return $this->connection->fetchAllAssociative(
            self::SELECT_REQUEST . ' WHERE scope_type = :scope AND subject_user_id = :user'
    ...
    public function requestsForHome(
        string $homeId,
        int $limit,
        int $offset,
    ): array {
        return $this->connection->fetchAllAssociative(
            self::SELECT_REQUEST . ' WHERE scope_type = :scope AND home_id = :home'
                . ' ORDER BY created_at DESC, id DESC LIMIT :limit OFFSET :offset',
            [
                'scope' => 'home',
                'home' => $homeId,
        ...
        return $this->connection->executeStatement(
            ('UPDATE data_governance_requests
             SET status = :completed, '
                . 'active_key = NULL, artifact_reference = :reference,
                 '
                . 'artifact_nonce = :nonce, artifact_sha256 = :sha256, artifact_size = '
                . ':size,
                 artifact_expires_at = :expires, completed_at = '
                . ':at,
                 revision = revision + 1, updated_at = :at'
                . '
             WHERE id = :id AND status = :processing AND revision = '
                . ':revision'),
            [
                'completed' => 'completed',
        ...
        return $this->connection->executeStatement(
            ('UPDATE data_governance_requests
             SET download_token_hash = '
                . ':hash, download_token_expires_at = :expires,
                 '
        ...
        $updated = $this->connection->executeStatement(
            ('UPDATE data_governance_requests
             ...
             WHERE id = :id AND status = :completed AND '
               ...
               AND downloaded_at IS NULL AND'
                . ' artifact_expires_at > :at
    ...
    public function transition(
        ...
        if (
            ...
            throw new \InvalidArgumentException(
                'Unsupported data-governance request transition.',
            );
        }
        ...
        return $this->connection->executeStatement(
            ('UPDATE data_governance_requests
             SET status = :next_status, '
                ...
                . ':is_processing = 1 THEN :at ELSE started_at END,
                 '
                . 'completed_at = CASE WHEN :is_terminal = 1 THEN :at ELSE completed_at '
                . 'END,
                ...
                . ' = :failure,
                 revision = revision + 1, updated_at = :at'
                . '
             WHERE id = :id AND status = :current_status AND revision ='
                . ' :revision'),
            [
                'next_status' => $toStatus,
                'active_key' => $isTerminal
                    ? null
                    : 'active',
                ...
                'artifact_expires' => $artifactExpiresAt === null
                    ? null
                    : $this->date($artifactExpiresAt),
