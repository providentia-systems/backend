...
final readonly class DbalCatalogImportHomeProductGateway implements CatalogImportHomeProductGateway
{
    public function __construct(
        ...
        private \Providentia\Access\Application\AccessService $access,
    ) {
    }
    ...
    public function matchingActiveId(
            ...
            $id = $this->connection->fetchOne(
                ('SELECT id FROM home_products
                 WHERE home_id = :home AND '
                    . 'product_id = :product AND status = :active
                   AND '
                    . '((pack_id = :pack) OR (pack_id IS NULL AND :pack IS NULL))'
                    . '
                 ORDER BY id LIMIT 1'),
                [
                    'home' => $homeId,
        ...
        $id = $this->connection->fetchOne(
            ('SELECT id FROM home_products
             WHERE home_id = :home AND '
                . 'normalized_private_name = :name AND status = :active
             ORDER '
                . 'BY id LIMIT 1'),
            [
                'home' => $homeId,
                'name' => $normalizedPrivateName,
                'active' => 'active',
            ],
        );
        return $id === false
