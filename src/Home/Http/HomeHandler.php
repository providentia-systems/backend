...
final class HomeHandler implements RequestHandlerInterface
{
    ...
    public function handle(
        ...
        return match ($this->action) {
            ...
            'memberships' => new JsonResponse(
                [
                    'data' => $this->homes->memberships($identity, $homeId),
                ],
            ),
            ...
            'configure-permissions' => new JsonResponse(
                $this->homes->configureRolePermissions(
                    $identity,
                    $homeId,
            ...
            'invitations' => new JsonResponse(
                [
                    'data' => $this->homes->invitations($identity, $homeId),
                ],
            ),
            ...
            'member-permissions' => new JsonResponse(
                $this->homes->memberPermissions(
                    $identity,
                    $homeId,
            ...
            'pending-invitations' => new JsonResponse(
                [
                    'data' => $this->homes->pendingInvitations($identity),
                ],
            ),
            'accept-invitation-by-id' => new JsonResponse(
                $this->homes->acceptInvitationById(
                    $identity,
            ...
            'propose-ownership-transfer' => new JsonResponse(
                $this->homes->proposeOwnershipTransfer(
                    $identity,
                    $homeId,
            ...
            'accept-ownership-transfer' => $this->ownershipTransition($identity, $homeId, $request, $body, 'accept'),
            'reject-ownership-transfer' => $this->ownershipTransition($identity, $homeId, $request, $body, 'reject'),
            'revoke-ownership-transfer' => $this->ownershipTransition($identity, $homeId, $request, $body, 'revoke'),
    ...
    private function declineInvitation(
    ...
    /**
     * @param array<string, mixed> $body */
    private function saveMemberPermissions(
        AuthenticatedIdentity $identity,
        string $homeId,
        ServerRequestInterface $request,
        array $body,
    ): ResponseInterface {
        $this->homes->saveMemberPermissions(
            $identity,
            $homeId,
            (string) $request->getAttribute('userId', ''),
            $body,
        ...
        foreach (array_keys($body) as $field) {
            if (!in_array($field, $allowed, true)) {
                throw new Problem(
                    422,
                    'Validation failed',
                    'Unknown home setting: ' . $field . '.',
                );
            }
        }
        $mutable = array_intersect(
            ['name', 'locale', 'currency', 'timezone'],
            array_keys($body),
        );
        if ($mutable === []) {
            throw new Problem(
                422,
                'Validation failed',
                'At least one home setting must be supplied.',
            );
        }
