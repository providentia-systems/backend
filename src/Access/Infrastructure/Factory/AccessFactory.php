    ...
    public function __invoke(
        ...
        return match (true) {
            ...
            $name === OperatorWorkspaceService::class => new OperatorWorkspaceService(
                ...
                $container->get(AccountProfileStore::class),
                $container->get(TransactionManager::class),
                $container->get(Clock::class),
            ),
            $name === ProfileMediaService::class => new ProfileMediaService(
                $container->get(ProfileMediaStore::class),
                $container->get(AccountProfileStore::class),
            ...
            str_starts_with($name, 'operator-workspace.') => new OperatorWorkspaceHandler(
                $container->get(OperatorWorkspaceService::class),
                substr($name, 19),
            ),
            str_starts_with($name, 'profile-media.') => new ProfileMediaHandler(
                $container->get(ProfileMediaService::class),
                substr($name, 14),
            ),
            $name === DbalAccessStore::class => new DbalAccessStore(
                $container->get(Connection::class),
                $container->get(Clock::class),
            ...
            $name === AccessService::class => new AccessService(
                $container->get(AccessStore::class),
                $container->get(TransactionManager::class),
                $container->get(UuidGenerator::class),
            ),
            $name === CountryService::class => new CountryService(
                $container->get(CountryStore::class),
                $container->get(AccessService::class),
                $container->get(AccessStore::class),
                $container->get(Clock::class),
                $container->get(TransactionManager::class),
            ...
            $name === EmailCodeService::class => new EmailCodeService(
            ...
            $name === EmailLoginService::class => new EmailLoginService(
                $container->get(EmailCodeService::class),
            ...
            $name === AccountProfileService::class => new AccountProfileService(
                $container->get(AccountProfileStore::class),
                $container->get(IdentityStore::class),
            ...
            str_starts_with($name, 'email-login.') => new EmailLoginHandler(
                $container->get(EmailLoginService::class),
                $name === 'email-login.verify',
                $config['identity']['cookie_secure'],
            ),
            str_starts_with($name, 'profile.') => new AccountProfileHandler(
                $container->get(AccountProfileService::class),
                substr($name, 8),
            ),
            str_starts_with($name, 'access.') => new AccessHandler(
                $container->get(AccessService::class),
                substr($name, 7),
            ),
            str_starts_with($name, 'country.') => new CountryHandler(
                $container->get(CountryService::class),
                substr($name, 8),
            ),
