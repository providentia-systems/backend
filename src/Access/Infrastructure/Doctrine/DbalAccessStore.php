...
final class DbalAccessStore implements AccessStore
{
    ...
    public function groups(?string $scope = null): array
    {
        $rows = $this->connection->fetchAllAssociative(
            'SELECT * FROM access_groups' . ($scope === null
                ? ''
                : ' WHERE scope = :scope') . ' ORDER BY scope, name',
            $scope === null
                ? []
                : ['scope' => $scope],
        );
    ...
    public function group(string $id): ?array
    {
        $row = $this->connection->fetchAssociative(
            'SELECT * FROM access_groups WHERE id = :id',
            ['id' => $id],
        );
        return $row === false
            ? null
            : $this->decodeGroup($row);
    }
    ...
    public function saveGroup(
        ...
        $values = [
            ...
            'delegable_json' => json_encode(
                $group['delegablePermissions'],
                JSON_THROW_ON_ERROR,
            ),
            'role_permissions_json'
                => json_encode($group['rolePermissions'], JSON_THROW_ON_ERROR),
    ...
    public function assignment(
        ...
        $row = $this->connection->fetchAssociative(
            ('SELECT g.*, a.group_id, a.revision AS assignment_revision
             '
                . 'FROM access_assignments a INNER JOIN access_groups g ON g.id = '
                . 'a.group_id
             WHERE a.scope = :scope AND a.subject_id = '
                . ':subject AND g.scope = :scope
               AND (:scope <> :admin_scope'
                . ' OR EXISTS (SELECT 1 FROM administrator_requests r WHERE r.user_id = '
                . 'a.subject_id AND r.status = :approved)
                    OR EXISTS '
                . '(SELECT 1 FROM system_owner_bootstrap b WHERE b.user_id = a.subject_id '
                . 'AND a.group_id = :owner_group))')
                . ($currentRead
            ...
            [
                'scope' => $scope,
                'subject' => $subjectId,
                'admin_scope' => FeatureCatalog::ADMIN,
                'approved' => 'approved',
                'owner_group' => FeatureCatalog::SYSTEM_OWNER,
            ],
        );
        if ($row === false) {
            return null;
        ...
        return [
            ...
            'groupRevision' => (int) $row['revision'],
    ...
    public function lockSubject(
        ...
        if (!$this->connection->isTransactionActive()) {
            throw new \LogicException(
                'Access capacity must be checked within a transaction.',
            );
        }
        $table = $scope === FeatureCatalog::HOME
            ? 'homes'
            : 'users';
        ...
        if (
            ...
            throw new Problem(
                404,
                'Not found',
                'The requested account or home is unavailable.',
            );
        }
    }
    ...
    public function resourceCount(
        ...
        $params = ['id' => $subjectId];
        $sql = match ($resource) {
            'homes.owned' => ('SELECT COUNT(*) FROM home_memberships m INNER JOIN homes h ON h.id = '
                . 'm.home_id
                WHERE m.user_id = :id AND m.role = \'owner\' AND'
                . ' m.status = \'active\' AND h.status = \'active\''),
            'homes.joined' => ('SELECT COUNT(*) FROM home_memberships m INNER JOIN homes h ON h.id = '
                . 'm.home_id
                WHERE m.user_id = :id AND m.role <> \'owner\' '
                . 'AND m.status = \'active\' AND h.status = \'active\''),
            'members.total' => ('SELECT COUNT(*) FROM home_memberships WHERE home_id = :id AND status = '
                . '\'active\''),
    ...
    {
        $row = $this->connection->fetchAssociative(
            ('SELECT * FROM member_permission_overrides WHERE home_id = :home AND '
                . 'user_id = :user'),
    ...
    public function auditEvents(int $limit, int $offset): array
    {
        return $this->connection->fetchAllAssociative(
            ('SELECT id, actor_user_id AS actorUserId, action, scope, subject_id AS '
                . 'subjectId,
                    details_json AS details, created_at AS '
                . 'createdAt
             FROM platform_audit_events ORDER BY created_at '
                . 'DESC, id DESC LIMIT ') . max(1, min(100, $limit)) . ' OFFSET '
                . max(0, $offset),
        );
    }
    ...
    private function decodeGroup(array $row): array
    {
        return [
            ...
            'rolePermissions' => json_decode(
                (string) $row['role_permissions_json'],
                true,
                32,
                JSON_THROW_ON_ERROR,
            ),
            ...
            'delegablePermissions' => json_decode(
                (string) $row['delegable_json'],
                true,
                32,
                JSON_THROW_ON_ERROR,
            ),
