...
{
    public function __construct(private readonly AccessService $access, private readonly string $action)
    {
    }
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($this->action === 'assign') {
            $this->access->assign($identity, (string) $request->getAttribute('scope', ''), (string) $request->getAttribute('subjectId', ''), (string) ($body['groupId'] ?? ''), (int) ($body['expectedRevision'] ?? 0));
        ...
        if ($this->action === 'catalog') {
            ...
            foreach ([FeatureCatalog::ACCOUNT, FeatureCatalog::HOME, FeatureCatalog::ADMIN] as $scope) {
                $scopes[] = ['scope' => $scope, 'features' => FeatureCatalog::features($scope), 'limits' => FeatureCatalog::limits($scope)];
            }
            return new JsonResponse(['data' => $scopes]);
        }
        return new JsonResponse(match ($this->action) {
            'list' => ['data' => $this->access->groups($identity, isset($query['scope']) ? (string) $query['scope'] : null)],
            'create', 'update' => $this->access->saveGroup($identity, $this->action === 'create' ? null : (string) $request->getAttribute('groupId', ''), $body),
