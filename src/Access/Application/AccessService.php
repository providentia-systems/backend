...
final class AccessService
{
    ...
    public function effective(
        ...
        $assignment = $this->store->assignment($scope, $subjectId, $currentRead);
        return [
            'scope' => $scope,
            'subjectId' => $subjectId,
    ...
    public function requireAdmin(
        ...
        if (
            ...
            throw new Problem(
                403,
                'Permission required',
                'Your administrator group does not permit this action.',
            );
        }
    }
    ...
    public function groups(
        ...
        if (
            ...
            ) && !($scope !== FeatureCatalog::ADMIN && $this->allows(
                FeatureCatalog::ADMIN,
                $identity->userId,
                'countries.manage',
            ))
            ...
            throw new Problem(
                403,
                'Permission required',
                'Your administrator group cannot inspect these groups.',
            );
        }
    ...
    public function saveGroup(
        ...
        if (
            FeatureCatalog::features($scope) === [] || $name === '' || mb_strlen($name) > 120
            || mb_strlen($description) > 1000
        ) {
            throw new Problem(
                422,
                'Invalid group',
                'Provide a supported scope and a name of up to 120 characters.',
            );
        }
        $features = $this->booleanMap(
            $input['features'] ?? [],
            FeatureCatalog::features($scope),
        );
        $limits = $input['limits'] ?? [];
        if (!is_array($limits)) {
        ...
        foreach ($limits as $key => $value) {
            if (!in_array($key, FeatureCatalog::limits($scope), true) || !is_int($value) || $value < -1) {
                throw new Problem(
                    422,
                    'Invalid limit',
                    'Use a known limit and a nonnegative integer, or -1 for unlimited.',
                );
            }
        }
        ...
        if (!is_array($delegable) || !array_is_list($delegable)) {
            throw new Problem(
                422,
                'Invalid delegation',
                'Delegable permissions must be a list.',
            );
        }
        foreach ($delegable as $permission) {
            if (
                $scope !== FeatureCatalog::HOME || !is_string($permission) || !in_array(
                    $permission,
                    FeatureCatalog::features($scope),
                    true,
                )
                ...
                throw new Problem(
                    422,
                    'Invalid delegation',
                    'Only known home permissions can be delegated.',
                );
            }
        }
        ...
        if (!is_array($rolePermissions)) {
            throw new Problem(
                ...
                'Provide role permission lists.',
            );
        }
        foreach ($rolePermissions as $role => $permissions) {
            if (
                $scope !== FeatureCatalog::HOME || !in_array($role, ['manager', 'member', 'viewer'], true)
                || !is_array($permissions) || !array_is_list($permissions)
            ) {
                throw new Problem(
                    422,
                    'Invalid role defaults',
                    'Only home manager, member and viewer defaults are supported.',
                );
            }
            foreach ($permissions as $permission) {
                if (
                    !is_string($permission) || !in_array(
                        $permission,
                    ...
                    throw new Problem(
                        ...
                        'Choose a known home permission.',
                    );
                }
            }
        }
        ...
        if (($previous['protected'] ?? false) || $previous !== null && $previous['scope'] !== $scope) {
            throw new Problem(
                409,
                'Protected group',
                'The system-owner group and group scope cannot be changed.',
            );
        }
        ...
        $group = [
            ...
            'features' => $features,
            'limits' => $limits,
            'delegablePermissions' => array_values(array_unique($delegable)),
            'protected' => false,
            'revision' => $revision + 1,
            'rolePermissions' => $rolePermissions,
        ];
        $this->transactions->transactional(
            function () use ($identity, $group, $revision): void {
                if (!$this->store->saveGroup($group, $revision)) {
                    throw new Problem(
                        409,
                        'Revision conflict',
                        'Reload the group before saving.',
                    );
                }
                $this->store->audit(
                    $identity->userId,
     ...
     */
    public function deleteGroup(AuthenticatedIdentity $identity, string $id, array $input): array
    {
        $this->requireAdmin($identity, 'groups.manage');
        ...
        $this->transactions->transactional(function () use ($identity, $id, $revision, $reason): void {
    ...
    public function assign(
        ...
        $permission = match ($scope) {
            ...
            default => throw new Problem(
                422,
                'Invalid scope',
                'The group scope is not supported.',
            ),
        };
        $this->requireAdmin($identity, $permission);
        ...
        if (
            $group === null || $group['scope'] !== $scope || ($group['protected'] ?? false)
            || ($existing['groupId'] ?? null) === FeatureCatalog::SYSTEM_OWNER
        ) {
            throw new Problem(
                422,
                'Invalid assignment',
                'Choose an editable group of the correct scope.',
            );
        }
        if ($scope === FeatureCatalog::ADMIN) {
            ...
            function () use (
                ...
                if (!$this->store->assign($scope, $subjectId, $groupId, $expectedRevision)) {
                    throw new Problem(
                        409,
                        'Revision conflict',
                        'Reload the assignment before changing it.',
                    );
                }
                $this->store->audit(
                    $identity->userId,
    ...
    public function initialize(
        ...
        if ($group === null || $group['scope'] !== $scope) {
            throw new Problem(
                ...
                'The operator must configure a valid default group.',
            );
        }
    ...
    public function requireCapacity(
        string $scope,
        string $subjectId,
        string $resource,
        ...
        $this->store->lockSubject($scope, $subjectId);
        $maximum = (int) ($this->effective($scope, $subjectId, true)['limits'][$resource] ?? 0);
        if (
            $maximum !== -1
            && $this->store->resourceCount($scope, $subjectId, $resource) >= $maximum + ($alreadyInserted
            ? 1
            : 0)
        ) {
            throw new Problem(
                409,
                'Allowance reached',
                'The current group does not allow another ' . $resource . '.',
            );
        }
    }
    ...
    public function homePermission(
        ...
        $override = in_array($permission, $delegable, true)
            ? $this->memberPolicy($homeId, $userId)['permissions'][$permission] ?? null
            : null;
        return is_bool($override)
            ? $override
            : in_array($permission, $inherited, true);
    }
            ...
            function () use (
                ...
                if (!$this->store->saveMemberPolicy($homeId, $userId, $normalized, $expectedRevision)) {
                    throw new Problem(
                        409,
                        'Revision conflict',
                        'Reload the member permissions before saving.',
                    );
                }
                $this->store->audit(
                    $actor->userId,
        ...
        foreach ($value as $key => $enabled) {
            if (!is_string($key) || !in_array($key, $known, true) || !is_bool($enabled)) {
                throw new Problem(
                    422,
                    'Invalid permission',
                    'Only permitted boolean feature values can be changed.',
                );
            }
        }
        return $value;
