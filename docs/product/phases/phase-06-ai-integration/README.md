...
- EXIF-bearing input rejection and no server-side media persistence
...
No image bytes, data URLs, EXIF, provider credentials, hidden reasoning, or raw
provider error bodies are stored in the database, queue, audit export, or API
response.
